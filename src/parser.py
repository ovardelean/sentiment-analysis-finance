    ...
    def group_indicators(self, result, tokens):
        ...
        for i in range(0, len(result)):
            ...
            if result[i] in target:
                if i < len(result)-1 and result[i+1] in group_with:
                    new_result.append(result[i]+":"+result[i+1])
                    new_tokens.append(tokens[i]+" "+tokens[i+1])
                    had = True
                elif i > 0 and result[i-1] in group_with:
                    new_result.append(result[i]+":"+result[i-1])
                    new_tokens.append(tokens[i]+" "+tokens[i-1])
                    had = True
                elif i < len(result)-2 and not result[i+1] and result[i+2] in group_with:
                    new_result.append(result[i]+":"+result[i+2])
                    new_tokens.append(tokens[i]+" "+tokens[i+2])
                    had = True
                elif i > 1 and not result[i-1] and result[i-2] in group_with:
                    new_result.append(result[i]+":"+result[i-2])
                    new_tokens.append(tokens[i]+" "+tokens[i-2])
                    had = True
